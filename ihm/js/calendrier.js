	...
	calendar.prototype.updateDate = function(direction){
		if(this.timer == null){
			if(direction == "before"){
			//on calcule les dates suivante et précédente
			...
			//On récupère le mois actuel puis on vérifie que l'on est pas en janvier sinon on ajoute une année
	...
	calendar.prototype.SlideToLeft = function(){
		if((this.direction == null || this.direction == 'left') && this.opacite >= 100){
			var me = this ;
			//On vérifie la direction pour initialiser le positionnement
	...
	calendar.prototype.fadePic = function (current,up){
		...
		if (this.opacite< 100 && this.up){
			this.opacite=100;//remplacement de "this.opacite+=3;" pour éviter le fade
			this.IsIE?this.calendar.filters[0].opacity=this.opacite:this.calendar.style.opacity=this.opacite/100;
			var me = this;
			// appel fonction récursive, ici ne fonctionne que sur un tour grâce à la modification
